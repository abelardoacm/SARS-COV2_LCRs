...
getComplexity_moda <- function(x, data) {
  ...
  return(names(sort(summary(as.factor(tmp$complexity)), decreasing=T)[1]))
}
...
system("mkdir -p ../results/Proteome_complexityplots/")
...
outcomplot <- paste("../results/Proteome_complexityplots/",variante,"_proteome_complot.tiff",sep="")
...
for (p in proteinas){
  p2p = p
  outprotcomplot <- paste("../results/Proteome_complexityplots/",variante,"_",p2p,"_complot.tiff",sep="")
